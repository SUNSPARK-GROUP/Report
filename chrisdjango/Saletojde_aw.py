...
def set2jde(tday):
  ...
  for sales in cur206hd :
    ...
    cur2061hd.execute("SELECT A.go_no,a.no_sm,a.id_cust,a.applydate,a.sdatetime,convert(nvarchar(200),a.remark) as remark,B.USERID,B.TYPENO,"
                    +"B.SALETYPE,IsNull(B.BONNER,0) AS USERBON,ISNULL(B.CARNO,'')CARNO,B.USERIDNEW ,a.aDATE,a.apdate  "
                    ...
                    +ndate+"') as NVARCHAR), 3) as aDATE,SDATETIME,REMARK,APPLYDATE as apdate FROM WEBORDERHD WHERE GO_NO='"
                    +str(sales[0])+"')A,USERGROUP B WHERE A.ID_CUST=B.USERID")  #20191212 
    ...
    cur2061fl.execute("SELECT count(*) as q FROM (SELECT * FROM WEBORDERFL WHERE GO_NO='"+go_no+"')A,WEBART B WHERE A.ID_ITEM=B.ID_ITEM union all "
                    +" select 0-count(*) from (SELECT * FROM WEBORDERfl WHERE GO_NO='"+go_no+"') a  where ID_ITEM  in (SELECT  [id_item]  FROM [TGSalary].[dbo].[WEBBookingART]) ") #20191211
    fc=0
    odnos=0
    for flcount in cur2061fl:
      odnos=odnos+int(flcount[0])
    for sale in cur2061hd:
      if str(sale[1])=='None':#開始轉單
        ...
        if str(sales[4]).find('非正配單')==-1:#20191115
        ...
        if inbooking==1:#20191115
          cur206temp.execute(" UPDATE [WEBBookingOrder] set  resale='Y' WHERE [applydate]<='"+t_str+"' and [ID_CUST]='"+cid+"' and resale='N' ")#20191031 20191112 [applydate]='"+t_str+"' 
          cur206temp.commit()
  #f.close()
  return salel
