...
def CONORACLE(SqlStr):
  #f=open(r'D:\chrisdjango\error_o.txt','w')
...
def F43121item(request):
  ...
  f=open('C:\\Users\\chris\\chrisdjango\\43121Ierror.txt','w')
  try:
  ...
  try:
    ...
    if request.GET['commit']== '查詢發票':  
      evidata=CONORACLE("SELECT a.* ,(a.NEW_RPATXA-a.RPATXA) gap FROM (SELECT RPAN8,TO_CHAR(rpalph) rpalph,SUM(RPAG) RPAG,SUM(RPATXA) RPATXA,ROUND(SUM(RPAG)/1.05,0) NEW_RPATXA"
                  ...
                  +"WHERE RPDGJ>='"+sd+"' AND RPDGJ<='"+ed+"' AND rpurrf='               ' and RPDCT in('RI','RM')  AND RPDCT like '%"+RPDCT+"%' AND RPAN8 LIKE '"+AN8+"%' "#and RPAN8='1000548' and RPAN8 like '4%'
...
def pettycash(request):#零用金待轉項目查詢
  # f = open(r'D:\chrisdjango\cash.txt','w')
  context={}
